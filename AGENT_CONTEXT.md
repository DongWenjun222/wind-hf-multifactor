...
10. 综合模型训练支持样本时间衰减加权，默认让越靠近预测时点的训练样本权重越高，同时保留类别权重。
11. 综合模型输出验证集到最终测试集的衰减诊断，用于识别验证有效但最终留存测试明显失效的模型。
12. 综合策略加入了持仓规则优化，包括最小持仓期、反转冷却、小变化忽略和可选平滑。
13. 综合策略加入了市场状态分层诊断，默认按趋势、波动、流动性、交易时段和组合状态输出表现报告；状态过滤默认关闭，需要显式配置后才会影响交易。
...
2. 训练样本时间衰减权重：已接入综合模型训练，可继续对半衰期和最低权重做验证集对比。
